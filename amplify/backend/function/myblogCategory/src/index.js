...
app.put("/category/:nickname", async (req, res) => {
  // set cors policy
  res.setHeader("Access-Control-Allow-origin", "*");
  res.setHeader("Access-Control-Allow-Credentials", "true");
  ...
  try {
    ...
    const { Count, Items } = await ddbDocClient.send(userQueryCommand);
    ...
    if (id !== userID) {
      res.status(403).json({
        message: "Access other user's information.",
      });
    }
    ...
    const { Attributes } = await ddbDocClient.send(categoryPutCommand);
